...
func main() {
	...
	walletAddr := p.GetString("wallet.address", "0x0000")
	harvestToken := p.GetString("harvest.token", "0x0000")
	masterChefContract := p.GetString("masterChef.address", "0x0000")
	...
	for {
		...
		pendingTokenCount := new(big.Float).Quo(new(big.Float).SetInt(&pendingTokenWeiCount), big.NewFloat(1000000000000000000))
		totalHarvestValue := new(big.Float).Mul(tokenUSDPrice, pendingTokenCount)
		log.Println("Total harvest value: $" + totalHarvestValue.String())
		if totalHarvestValue.Cmp(new(big.Float).SetInt64(busdThreshold)) > 1 {
			log.Println("Harvesting..")
			harvestTx := utils.Harvest(poolId, masterChefContract)
			log.Println("Harvested token:" + harvestToken + " with tx:" + harvestTx)
			time.Sleep(10 * time.Second)

			balance := utils.BalanceOf(harvestToken, walletAddr)
			minBalance := new(big.Float).Quo(new(big.Float).SetInt(&balance), new(big.Float).SetFloat64(slippage))
			minBalanceInt := new(big.Int)
			minBalance.Int(minBalanceInt)

			now := time.Now()
			nanos := now.UnixNano() / 1000000
			utils.SwapApprovedTokenToBUSD(balance, *minBalanceInt, walletAddr, *new(big.Int).SetUint64(uint64(nanos)), harvestToken)
		}
